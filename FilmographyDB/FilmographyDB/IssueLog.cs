...
namespace FilmographyDB
{
    public partial class IssueLog : Form
    {
        ...
        {
            // TODO: данная строка кода позволяет загрузить данные в таблицу "sQL_FilmographyDataSet.IssueLog". При необходимости она может быть перемещена или удалена.
            this.issueLogTableAdapter.Fill(this.sQL_FilmographyDataSet.IssueLog);
        ...
        private void сохранитьИзмененияToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                dataGridView1.DataSource = sQL_FilmographyDataSet.IssueLog;//Записывает данные из DataGrid в DataSet
                issueLogTableAdapter.Update(sQL_FilmographyDataSet);//Обновляет БД
            }
            catch (Exception ex)
            {
        ...
        private void удалитьТекущуюЗаписьToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (CurMan.Count > 0)
            {
                ...
                issueLogTableAdapter.Update(sQL_FilmographyDataSet);
            }
        }
